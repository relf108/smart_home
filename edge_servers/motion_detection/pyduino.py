"""
Source: https://github.com/pearsonkyle/pyduino_webapi - but modified for extra functionality
...
class Arduino():
    ...
    def digital_play(self, pin_number, digital_value):
        """
        Declares a frequency to play on speaker on pin_number
        Internally sends b'PD{pin_number}:{digital_value}' over the serial
        connection
        """
        command = (''.join(('PD', str(pin_number), ':',
            str(digital_value)))).encode()
        self.conn.write(command)
